...
def solution2(new_id):
    ...
    print([st[-1] for i in range(6-len(st))])
...
# 마지막조건(7번) 실행 시간 비교
# 문자열 총 길이가 길어질수록 채워야하는 문자자릿수가 많아지며, for문으로 list 생성후 join 을 하면 속도가 느려진다.
from timeit import default_timer as timer
from datetime import timedelta

max_len = 10000

def compareTime(new_id):
  st = new_id
  start = timer()
  if len(st) <= 2 :
      st = st.ljust(max_len, st[-1:])
  end = timer()
  print(timedelta(seconds=end-start))

def compareTime2(new_id):
  st = new_id
  start = timer()
  st = st if len(st) > 2 else st + "".join([st[-1] for i in range(max_len-len(st))])
  end = timer()
  print(timedelta(seconds=end-start))
